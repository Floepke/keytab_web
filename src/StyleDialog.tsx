...
type NumericLayoutField = {
  [Field in LayoutField]: Layout[Field] extends number ? Field : never;
}[LayoutField];
type NumericFontField = "size_pt" | "x_offset" | "y_offset";
...
const NUMBER_FIELD_CONFIG: Record<NumericLayoutField, NumericConfig> = {
  // Page
  scale: { min: 0.25, max: 1, step: 0.01 },
  page_width_mm: { min: 50, max: 5000, step: 0.5 },
  page_height_mm: { min: 50, max: 10000, step: 0.5 },
  page_top_margin_mm: { min: 0, max: 100, step: 0.05 },
  page_bottom_margin_mm: { min: 0, max: 100, step: 0.05 },
  page_left_margin_mm: { min: 0, max: 100, step: 0.05 },
  page_right_margin_mm: { min: 0, max: 100, step: 0.05 },
  header_height_mm: { min: 0, max: 100, step: 0.05 },
  footer_height_mm: { min: 0, max: 100, step: 0.05 },
  // Notes
  note_stem_length_semitone: { min: 3, max: 20, step: 1 },
  note_stem_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  note_stopsign_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  note_continuation_dot_size_mm: { min: 0.05, max: 10, step: 0.05 },
  notehead_height_scaling: { min: 0.1, max: 3, step: 0.01 },
  note_width_scaling: { min: 0.05, max: 2, step: 0.01 },
  notehead_tilt: { min: 0, max: 0.5, step: 0.01 },
  beam_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  beam_corner_radius_mm: { min: 0, max: 5, step: 0.05 },
  grace_note_outline_width_mm: { min: 0.05, max: 5, step: 0.05 },
  grace_note_scale: { min: 0.05, max: 1, step: 0.05 },
  // Symbols
  pedal_symbol_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  pedal_background_padding_mm: { min: 0, max: 10, step: 0.05 },
  text_background_padding_mm: { min: 0, max: 20, step: 0.05 },
  slur_width_sides_mm: { min: 0.05, max: 5, step: 0.05 },
  slur_width_middle_mm: { min: 0.05, max: 5, step: 0.05 },
  hairpin_line_width_mm: { min: 0.05, max: 5, step: 0.05 },
  hairpin_width_mm: { min: 0.05, max: 20, step: 0.05 },
  dynamic_symbol_font_size_pt: { min: 4, max: 100, step: 0.5 },
  dynamic_symbol_background_padding_mm: { min: 0, max: 20, step: 0.05 },
  dynamic_rotation: { min: 0, max: 360, step: 1 },
  countline_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  // Grid
  grid_barline_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  grid_gridline_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  time_signature_indicator_lane_width_mm: { min: 0.05, max: 100, step: 0.05 },
  time_signature_indicator_guide_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  time_signature_indicator_divide_guide_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  measure_numbering_guide_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  // Staves
  stave_two_line_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  stave_three_line_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
  stave_clef_line_thickness_mm: { min: 0.05, max: 5, step: 0.05 },
};

const FONT_NUMBER_CONFIG: Record<NumericFontField, NumericConfig> = {
  size_pt: { min: 4, max: 200, step: 0.5 },
  x_offset: { min: -1000, max: 1000, step: 0.05 },
  y_offset: { min: -1000, max: 1000, step: 0.05 },
};
...
const STYLE_DIALOG_TAB_SESSION_KEY = "keytab-style-dialog-tab";
...
const initialStyleDialogTab = () => {
  try {
    const tab = sessionStorage.getItem(STYLE_DIALOG_TAB_SESSION_KEY);
    return CATEGORIES.some(([title]) => title === tab) ? tab : CATEGORIES[0][0];
  } catch {
...
function FontEditor({ value, onChange }: { value: Font; onChange: (value: Font) => void }) {
  ...
  const sizeConfig = FONT_NUMBER_CONFIG.size_pt;
  const horizontalOffsetConfig = FONT_NUMBER_CONFIG.x_offset;
  const verticalOffsetConfig = FONT_NUMBER_CONFIG.y_offset;
  return <div className="font-editor">
    ...
    <NumberInput aria-label="Font size" {...sizeConfig} value={value.size_pt} onChange={(next) => update("size_pt", String(next))} />
    ...
    <NumberInput aria-label="Font horizontal offset" {...horizontalOffsetConfig} value={value.x_offset} onChange={(next) => update("x_offset", String(next))} />
    <NumberInput aria-label="Font vertical offset" {...verticalOffsetConfig} value={value.y_offset} onChange={(next) => update("y_offset", String(next))} />
  </div>;
}
...
export function StyleDialog({ layout, onApply, onClose }: { layout: Layout; onApply: (layout: Layout) => void; onClose: () => void }) {
  ...
  const [activeTab, setActiveTab] = useState(initialStyleDialogTab);
  ...
  const selectTab = (tab: string) => {
    setActiveTab(tab);
    try {
      sessionStorage.setItem(STYLE_DIALOG_TAB_SESSION_KEY, tab);
    } catch {
      // The dialog still works if browser storage is unavailable.
    }
  };
  ...
  const editor = (field: LayoutField) => {
    ...
    if (typeof value === "number") {
      const config = NUMBER_FIELD_CONFIG[field as NumericLayoutField];
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="style-dialog" role="dialog" aria-modal="true" aria-label="Style" onMouseDown={(event) => event.stopPropagation()} onKeyDownCapture={(event) => event.stopPropagation()}>
      ...
      <div className="style-tabs" role="tablist">{CATEGORIES.map(([title]) => <button type="button" key={title} role="tab" aria-selected={activeTab === title} className={activeTab === title ? "active" : ""} onClick={() => selectTab(title)}>{title}</button>)}</div>
